...
@Named(value = "reclassificacaoController")
@ViewScoped
public class ReclassificacaoController implements Serializable {
	...
	public ReclassificacaoController() {
	...
	public List<Acao> completeAcoes(String query) {
		...
		allAcoes = service.acoesAutoCompleteRECLASSIFICACAO(query);
		return allAcoes;
	}
	...
	public void editarPagamentoTab2() {
		//pagamentoLancAuxiliar.lancamentoAcao.projetoRubrica
		
		if (pagamentoLancAuxiliar.getLancamentoAcao().getProjetoRubrica() == null) {
			service.addMessage("", "NÃO FOI POSSÍVEL SALVAR, ADICIONE UMA LINHA ORÇAMENTÁRIA AO LANÇAMENTO.", FacesMessage.SEVERITY_ERROR);
			return;
		}
		
		if (!calculatorRubricaRepositorio.verificarSaldoRubrica(pagamentoLancAuxiliar.getLancamentoAcao().getProjetoRubrica().getId(), pagamentoLancAuxiliar.getLancamentoAcao().getValor())) {
...
//				pagamentoLancAuxiliar.getLancamentoAcao().getProjetoRubrica().getProjeto().getId());
	...
	public void validarMultiplos(){
		service.validarMultiplos(lancamentosSelected);
		carregarLancamentosNãoProvisionados();
...
//		}
	...
	public void carregarLancamentosNãoProvisionados() {
		...
		lancamentos = service.getLancamentosNãoProvisionadosRECLASSIFICACAO(filtro);
