...
public class LocalidadeController implements Serializable {
	...
	public String salvar() {
		try {
			...
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Localidade", "Salvo com Sucesso!"));
		}catch (Exception e) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Localidade", "Não foi Possível Salvar Localidade"));
		}
	...
	public void delete(Localidade localidade) {
		if(localidadeService.delete(localidade)) {
			FacesContext.getCurrentInstance().addMessage("msgs", new FacesMessage(FacesMessage.SEVERITY_INFO, "Localidade", "Deletado com Sucesso!"));
		}else {
			FacesContext.getCurrentInstance().addMessage("msgs", new FacesMessage(FacesMessage.SEVERITY_FATAL, "Localidade", "Não foi Possivel deletar Localidade!"));
		}
		carregaLocalidade();
	}
