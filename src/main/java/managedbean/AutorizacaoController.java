...
@Named(value = "autorizacao_controller")
@ViewScoped
public class AutorizacaoController implements Serializable {
	...
	public void autorizar() {
		service.autorizar(identificacoes, usuarioSessao.getIdColadorador());
		carregarLista();
	}

	public void prepararDesautorizacao() {
		motivoRecusa = "";
		if (identificacoes != null && identificacoes.size() >= 1) {
			executeScript("PF('dlg_motivo').show();");
		} else {
	...
	public void desautorizar() {
		service.desautorizar(identificacoes, usuarioSessao.getIdColadorador(), motivoRecusa);
		carregarLista();
	...
	public void selecionarAprovacao(Lancamento lancamento) {
		...
		if (lancamento.getSelecionado()) {
			identificacoes.add(lancamento.getId());
		} else {
			identificacoes.remove(lancamento.getId());
		}
